...
2400	IF (PRSCON < 1) THEN
                GO TO 2700
        ELSE IF(PRSCON == 1) THEN
                GO TO 2550
        ELSE
                GO TO 2500
        END IF  ! parser reset?
...
C
C1025	CALL IDATE(SHOUR,SMIN,SSEC)		! get date (and toss).
C	I=(SHOUR*64)+(SMIN*8)+SSEC		! first seed
1025	I=Time8()				! better than nothing
