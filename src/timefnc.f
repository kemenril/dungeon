...
7000	DO 7100 I=1,OLNT			! find burning object
	  IF(AND((RECEP.EQ.OCAN(I)),(AND(OFLAG1(I),FLAMBT).NE.0)))
...
8200	DO 8250 I=1,OLNT			! blew wrong room.
	  IF(AND(QHERE(I,BR),(AND(OFLAG1(I),TAKEBT).NE.0)))
...
CD	TYPE 10,H,V,RMK,HFLG,OUT
CD10	FORMAT(' BLOW 10-- ',3I7,L7,I7)
...
100	ATT=MAX0(1,FIGHTS(H,.TRUE.))		! get his strength.
	...
	DO 200 I=1,OLNT				! search villain.
	  IF(AND((OCAN(I).EQ.V),(AND(OFLAG2(I),WEAPBT).NE.0)))
	1	DWEAP=I
200	CONTINUE
...
C
2000	CONTINUE
CD	TYPE 2050,ATT,OA,DEF,OD,DWEAP
CD2050	FORMAT(' BLOW 2050-- ',5I7)
...
CD	TYPE 2650,RES,MI,I,J,MBASE
CD2650	FORMAT(' BLOW 2650-- ',5I7)
...
20	FORMAT(' THIEFD-- IN ADV ROOM')
	...
	RETURN
C
1150	IF(OR((RHERE.EQ.0),(AND(OFLAG2(THIEF),FITEBT).EQ.0)))
